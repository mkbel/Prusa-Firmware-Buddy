...
#ifdef FOURIER_SERIES_OUTPUT_SAMPLES
    constexpr size_t max_len = 10 + 6 * 3 + 11 * 3 + 6 + 2 + 1; // 59
    char buff[max_len];
    snprintf(buff, max_len, "%lu %hd %hd %hd %ld %ld %ld\r\n", sample.timestamp, sample.val[0], sample.val[1], sample.val[2], sample.pos[0], sample.pos[1], sample.pos[2]);
    const uint32_t len = strnlen(buff, max_len);
    const uint32_t written = tud_cdc_n_write(0, buff, len);
    if (written < len) {
        bsod("failed to write to USB CDC");
    }
    #if 0
		tud_cdc_write_flush();
    #endif
